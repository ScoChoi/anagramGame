...
def check():
    ...
    if matched and in_jumble and not (text in matches):
        ...
        if len(matches) >= flask.session["target_count"]:
            rslt = {"new_word": 'success'}
            return flask.jsonify(result=rslt)
        else:
            rslt = {"new_word": 'new'}
            return flask.jsonify(result=rslt)
    elif text in matches:
        rslt = {"new_word": 'fail', "message": "You already found {}".format(text)}
        return flask.jsonify(result=rslt)
    elif not matched:
        rslt = {"new_word": 'fail', "message": "{} isn't in the list of words".format(text)}
        return flask.jsonify(result=rslt)
    elif not in_jumble:
        rslt = {"new_word": 'fail', "message": '"{}" can\'t be made from the letters {}'.format(text, jumble)}
        return flask.jsonify(result=rslt)
    else:
